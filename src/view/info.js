...
const getTripInfoTitle = (sortedEvents) => {
...
const getTripInfoDates = (sortedEvents) => {
  if (sortedEvents.length === 0) {
    return ``;
  }
  return `
...
const createInfoTemplate = (events) => {
  ...
  return `<section class="trip-main__trip-info  trip-info ${getEmptyDataClassName(events)}">
      <div class="trip-info__main">
        <h1 class="trip-info__title">${getTripInfoTitle(sortedEvents)}</h1>
        <p class="trip-info__dates">${getTripInfoDates(sortedEvents)}</p>
      </div>
